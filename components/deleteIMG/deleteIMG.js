...
function Grid() {
    ...
    const fetchDivisions = async () => {
      try {
        const response = await axios.get(
          "http://final-cam-server.vercel.app/image/divisions"
        );
  ...
  useEffect(() => {
    const fetchImageIds = async () => {
      if (selectedDivision !== null) {
        try {
          const response = await axios.get(
            `http://final-cam-server.vercel.app/image/divisions/${selectedDivision}`
          );
      ...
      if (imageIds.length > 0) {
        try {
          ...
          for (const idObj of imageIds) {
            const id = idObj._id;
            const response = await axios.get(
              `http://final-cam-server.vercel.app/image/fetch/${id}`,
              {
                responseType: "blob",
              }
            );
  ...
  const deleteImage = async (id) => {
    try {
      await axios.delete(`http://final-cam-server.vercel.app/image/delete/${id}`);
