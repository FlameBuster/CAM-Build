    ...
    const fetchDivisions = async () => {
      try {
        const response = await axios.get(
          "http://final-cam-server.vercel.app/video/divisions"
        );
  ...
  useEffect(() => {
    const fetchVideoIds = async () => {
      if (selectedDivision !== null) {
        try {
          const response = await axios.get(
            `http://final-cam-server.vercel.app/video/divisions/${selectedDivision}`
          );
      ...
      if (videoIds.length > 0) {
        try {
          ...
          for (const idObj of videoIds) {
            ...
            const response = await axios.get(
              `http://final-cam-server.vercel.app/video/fetch/${id}`,
              {
